...
const EmailVerificationPage = () => {
  ...
  return (
    <>
     <div className='max-w-md w-full bg-gray-800 mx-auto bg-opacity-50 backdrop-filter backdrop-blur-2xl rounded-xl shadow-xl overflow-hidden'>
      <motion.div initial={{opacity : 0, y : 20}}
      animate={{opacity : 1, y : 0}}
      transition={{duration : 0.5}}
      className="bg-gray-800 bg-opacity-50 backdrop-filter backdrop-blur-2xl rounded-2xl shadow-2xl p-8 w-full max-w-md">
            <h2 className='text-3xl font-bold mb-6 text-center bg-gradient-to-r from-green-400 to-emerald-500 bg-clip-text'>Verify Your Email</h2>
              ...
              <form onSubmit={handleSubmit} className='space-y-6'>
                <div className="flex justify-between">
                  {code.map((digit, index) =>(
                    ...
                    className='w-12 h-12 text-2xl font-bold bg-gray-700 text-center text-white border-2 border-gray-600 rounded-lg focus:border-green-500 focus:outline-none
                    '
                    />
                  ))}; 
                </div>
